...
class AIAgent:
    ...
    def __init__(
        ...
        **kwargs,
        ...
        # Provider fallback chain — used when the primary backend fails.
        # Config shapes:
        #   {"provider": "openrouter", "model": "anthropic/claude-sonnet-4"}
        #   - or -
        #   [{"provider": "...", "model": "..."}, {"provider": "...", "model": "..."}]
        self._fallback_chain = self._coerce_fallback_chain(fallback_model)
        self._fallback_index = 0
        self._fallback_model = self._fallback_chain[0] if self._fallback_chain else None
        self._fallback_activated = False
        if self._fallback_chain:
            fb_p = self._fallback_chain[0].get("provider", "")
            fb_m = self._fallback_chain[0].get("model", "")
            if fb_p and fb_m and not self.quiet_mode:
                print("🔄 Fallback chain: " + ", ".join(
                    f"{entry.get('model')} via {entry.get('provider')}" for entry in self._fallback_chain
                ))
    ...
    @staticmethod
    def _coerce_fallback_chain(raw_fallback: Optional[Any]) -> list[Dict[str, str]]:
        """Normalize legacy/single and multi-entry fallback configurations."""
        if isinstance(raw_fallback, dict):
            entries = [raw_fallback]
    ...
    def _try_activate_fallback(self) -> bool:
        ...
        while self._fallback_index < len(self._fallback_chain):
            ...
            try:
                ...
                if fb_client is None:
                    logging.warning(
                        "Fallback #%s to %s failed: provider not configured",
                ...
                print(
                    f"{self.log_prefix}🔄 Primary model failed — switching to fallback #{self._fallback_index}: "
                ...
                logging.error(
                    "Failed to activate fallback #%s (%s:%s): %s",
                    self._fallback_index,
                    fb_provider,
                    fb_model,
                    e,
